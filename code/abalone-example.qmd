...
# creating new object to clean abalone data from ab dataset
abalone_clean <- abalone |> 
  # cleaning columns
  clean_names() |> 
  # keep only columns of mean pH and change in area columns
...
# base layer: ggplot
ggplot(data = abalone_clean, # using abalone_clean data frame
       aes(x = mean_ph, # x- axis:  pH
           y = change_in_area)) + # y-axis: shell surface area change
...
**The missing value comes from a pair of tanks in which the pH sensor malfunctioned, so there is no pH recording from those tanks.**  
...
par(mfrow = c(2, 2))   # displays plots in 2 x 2 grid
...
`x`, `predicted`, `std_error`, `conf.low`, `conf.high`, `group` (not going to worry about `group` for now
)
...
**At pH = 8, the model predicts abalone shell growth of 5.36 [95% CI: 5.08, 5.64] mm^2^ d^-1^.**
